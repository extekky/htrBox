...
export function useShader(canvasRef: React.RefObject<HTMLCanvasElement | null>) {
    ...
    useEffect(() => {
        ...
        function buildColors(): Float32Array {
            ...
            const blend = (c: [number, number, number], t: number): [number, number, number] =>
                [c[0] * t + bg[0] * (1 - t), c[1] * t + bg[1] * (1 - t), c[2] * t + bg[2] * (1 - t)];
