 ...
 * - Возвращает массив бакетов с трафиком за 5-минутные интервалы.
 */
export function fetchUserTraffic(username: string, days: number): Promise<TrafficBucketResponse[]> {
    return get<TrafficBucketResponse[]>(`/traffic/user/${encodeURIComponent(username)}?days=${days}`);
}