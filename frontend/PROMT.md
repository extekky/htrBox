...
## Правило D — цветовые классы

Прежде чем писать сырой цветовой класс в слот `xStls.ts`, пройди проверку по порядку:

1. **`colorScheme`** — есть ли подходящий ключ в `variants.ts`?

   | Нужен цвет…          | Брать из…                   |
   | -------------------- | --------------------------- |
   | красный / ошибка     | `colorScheme.danger.solid`  |
   | жёлтый / предупрежд. | `colorScheme.warning.solid` |
   | зелёный / успех      | `colorScheme.success.solid` |
   | синий / информация   | `colorScheme.info.solid`    |
   | основной бренд       | `colorScheme.primary.solid` |
   | нейтральный серый    | `colorScheme.neutral.solid` |

   Каждый ключ `colorScheme` содержит: `bg`, `border`, `text`, `textMuted`, `solid`, `hover`. Выбирай нужный слот — не только `bg`.

2. **`tokens.ts`** — нет в `colorScheme`, но есть в `surface`, `typography` или другом токене? → использовать токен.

3. **Сырой класс** — только если ни `colorScheme`, ни токен не дают точного совпадения по смыслу и значению. Например, `bg-muted` (серый фон-плейсхолдер) не совпадает с `colorScheme.neutral.solid` (`bg-muted-foreground` — цвет текста).

Частые ошибки:

- `bg-red-500` → не писать напрямую, брать `colorScheme.danger.solid`
- `bg-amber-500` → не писать напрямую, брать `colorScheme.warning.solid`
- `text-emerald-500` → не писать напрямую, брать `colorScheme.success.text`
- `bg-blue-500/10` → не писать напрямую, брать `colorScheme.info.bg`
...
- Цветовые классы в слотах `cStyles/` — сначала проверить `colorScheme`, затем `tokens.ts`, сырой класс только если нет совпадения (см. Правило D)
